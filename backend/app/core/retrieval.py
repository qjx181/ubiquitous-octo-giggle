"""
<<<<<<< HEAD
backend/app/core/retrieval.py — 检索服务
=========================================
作用：整合向量检索和LLM生成，实现完整的RAG流程
原理：
  1. 将用户问题转为向量
  2. 从Milvus检索相关文本
  3. 调用LLM生成答案
...
<<<<<<< HEAD
    ...
    封装完整的RAG流程：
      Query → Embedding → Vector Search → LLM Generation → Answer
    
    使用示例：
        service = RetrievalService()
        result = service.query("公司的营业收入是多少？")
    """
    ...
    def __init__(self):
        ...
        自动获取Embedding、Milvus、LLM的客户端实例
        """
        self.embedding_service = get_embedding_service()
        self.milvus_client = get_milvus_client()
        # 使用智能调度器替代单一LLM客户端
        # 根据任务复杂度自动选择 SGLang 或 Ollama
...
<<<<<<< HEAD
        ...
        流程：
          1. 文本编码：将问题转为向量
          2. 向量检索：从Milvus召回相关chunk
          3. 上下文构建：组装检索结果作为LLM上下文
          4. 答案生成：调用LLM生成答案
          5. 结果组装：返回答案和引用来源
        ...
        参数：
            question: 用户问题
            top_k: 检索结果数量
            content_type: 内容类型过滤（"text"或"table"）
            history: 对话历史（多轮对话）
        ...
        示例返回：
            {
                "answer": "公司2023年营业收入为1000万元...",
                "sources": [
                    {"text": "...", "page": 15, "score": 0.85}
                ],
                "retrieval_results": [...]
            }
        """
        try:
            ...
            # 步骤2：向量检索
            logger.info("向量检索...")
            retrieval_results = self.milvus_client.search(
=======
        作用：执行完整的RAG查询（智能集成版）
...
<<<<<<< HEAD
            ...
            # 步骤4：生成答案（使用智能调度器）
            # 根据问题复杂度自动选择 SGLang（复杂）或 Ollama（简单）
            logger.info("生成答案...")
